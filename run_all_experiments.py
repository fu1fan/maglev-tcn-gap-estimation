...
def get_experiment_registry(base_cfg, data):
    """
    返回实验注册表。每个实验函数内部会：
      1. 重置随机种子 → 保证模型初始化与 DataLoader shuffle 一致
      2. 重新创建 DataLoader → 避免共享 loader 状态污染
      3. 训练 + 评估
    """
    ...
    # --- Baselines ---
    def _make_baseline_runner(name, model_fn):
        def run():
            m.set_seed(base_cfg.SEED)
            train_loader, test_loader = make_loaders(data["train_norm"], data["test_norm"], base_cfg)
            model = model_fn()
            metrics, _ = train_and_eval(model, train_loader, test_loader, base_cfg, y_min, y_max, name)
            return metrics
        return run

    registry["baseline_mlp"] = _make_baseline_runner(
        "baseline_mlp", lambda: MLPRegressor(in_ch, out_ch, base_cfg.WINDOW_LEN))
    registry["baseline_1dcnn"] = _make_baseline_runner(
        "baseline_1dcnn", lambda: CNN1DRegressor(in_ch, out_ch))
    registry["baseline_lstm"] = _make_baseline_runner(
        "baseline_lstm", lambda: LSTMRegressor(in_ch, out_ch))

    # --- 输入消融 ---
    def _make_ablation_runner(name, overrides, custom_build=None):
        def run():
            m.set_seed(base_cfg.SEED)
            ...
            train_loader, test_loader = make_loaders(
                abl_data["train_norm"], abl_data["test_norm"], abl_cfg)
            abl_in = len(abl_data["x_cols"])
            abl_out = len(abl_data["y_cols"])
            ...
            metrics, _ = train_and_eval(model, train_loader, test_loader, abl_cfg, abl_ymin, abl_ymax, name)
            return metrics
        return run

    registry["ablation_raw_only"] = _make_ablation_runner(
        "ablation_raw_only",
        {"USE_VOLTAGE": False, "USE_IAC": False, "USE_DIAC": False},
        build_no_diff)
    registry["ablation_no_ripple"] = _make_ablation_runner(
        "ablation_no_ripple",
        {"USE_IAC": False, "USE_DIAC": False})
    registry["ablation_no_duty"] = _make_ablation_runner(
        "ablation_no_duty",
        {"USE_VOLTAGE": False})
    registry["ablation_no_diff"] = _make_ablation_runner(
        "ablation_no_diff",
        {"USE_VOLTAGE": True, "USE_IAC": True, "USE_DIAC": False},
        build_no_diff)

    # --- TCN 宽度消融 ---
    def _make_width_runner(name, channels):
        def run():
            m.set_seed(base_cfg.SEED)
            train_loader, test_loader = make_loaders(data["train_norm"], data["test_norm"], base_cfg)
    ...
    for wname, wch in [("tcn_ch8", (8,)*5), ("tcn_ch16", (16,)*5), ("tcn_ch32", (32,)*5),
        ...
        registry[wname] = _make_width_runner(wname, wch)
...
def main():
    ...
    # 数据加载只做一次（确定性操作，不涉及随机数）
    data = prepare_data(base_cfg)
    in_ch = len(data["x_cols"])

    registry = get_experiment_registry(base_cfg, data)
